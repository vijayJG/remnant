...
def _get_record(path: str) -> dict | None:
    """Fetch the record for a given path."""
    with db() as conn:
        row = conn.execute(
            """SELECT r.* FROM records r
               JOIN reasons w ON w.record_id = r.id
               WHERE w.path = ?""",
            (path,),
        ).fetchone()
...
def set_reason(
    ...
    git = get_git_context(Path(resolved))

    with db() as conn:
        existing = conn.execute(
            "SELECT record_id FROM reasons WHERE path = ?", (resolved,)
        ).fetchone()

        if existing:
            conn.execute(
                """UPDATE records
                   SET body = ?, branch = ?, commit_hash = ?, updated_at = datetime('now')
                   WHERE id = ?""",
                (reason, git.get("branch"), git.get("commit_hash"), existing["record_id"]),
            )
            print_success(f"Updated reason for {path}")
        else:
            cursor = conn.execute(
                """INSERT INTO records
            ...
            conn.execute(
                ...
                (cursor.lastrowid, resolved),
            )
            print_success(f"Recorded reason for {path}")
...
def get_reason(
    ...
    init_db()
    resolved = _resolve_path(path)
    record = _get_record(resolved)

    if not record:
        print_warning(f"No reason recorded for {path}")
        print_info("Use: remnant why set <path> \"<reason>\"")
        raise typer.Exit(1)

    print_header(f"WHY: {path}")
    print_key_value("Reason",   record["body"])
    print_key_value("Recorded", record["created_at"][:10])
    if record["branch"]:
        print_key_value("Branch",  record["branch"])
...
def list_reasons(
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT r.path, rec.body, rec.updated_at
               FROM reasons r
               JOIN records rec ON rec.id = r.record_id
    ...
    print_table(
        ...
        rows=[[r["path"], r["body"][:60], r["updated_at"][:10]] for r in rows],
    )
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT r.path, rec.body, rec.updated_at
               FROM reasons r
               JOIN records rec ON rec.id = r.record_id
               WHERE rec.body LIKE ? OR r.path LIKE ?
               ORDER BY rec.updated_at DESC""",
            (f"%{query}%", f"%{query}%"),
        ).fetchall()
...
def remove_reason(
    ...
    print_success(f"Removed reason for {path}")
...
def history_reason(
    ...
    print_key_value("Commit",         record["commit_hash"] or "not recorded")
